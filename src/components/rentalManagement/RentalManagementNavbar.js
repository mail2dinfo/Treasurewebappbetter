...
import { RM_BASE_PATH, getRentalManagementAppMenuItems } from './rentalManagementMenuItems';
...
const RentalManagementNavbar = () => {
  ...
  const dashboardPath = `${RM_BASE_PATH}/dashboard`;
  const billingPath = `${RM_BASE_PATH}/billing`;
  const menuItems = getRentalManagementAppMenuItems(RM_BASE_PATH);
  ...
  const isItemActive = (item) => {
    ...
    if (item.id === 'home') {
      return (
        current === RM_BASE_PATH
        || current === `${RM_BASE_PATH}/`
        || current === `${RM_BASE_PATH}/dashboard`
      );
    }
    ...
    <header className="bg-gradient-to-r from-red-600 via-red-700 to-red-800 sticky top-0 z-50 shadow-lg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-14">
          <MyTreasureBrand to={dashboardPath} subtitle="Rental Management" inverse />
...
export default RentalManagementNavbar;
