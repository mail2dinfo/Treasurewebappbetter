...
const VehicleFinanceCollectionsPage = ({ onPaymentSuccess }) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (paymentType === 'partial' && remainingAmount > 0) {
                    alert(
                        `Partial payment collected! Remaining amount: ₹${Number(remainingAmount).toLocaleString('en-IN')} stays pending on the receivable (and is carried forward to the next due where applicable). Ledger entry recorded.`
                    );
                } else {
    ...
    const getLoanProgress = useCallback((loanId) => {
        ...
        return {
            total: loanReceivables.length,
            paid: loanReceivables.filter((r) => r.is_paid).length,
        };
    }, [receivables]);
